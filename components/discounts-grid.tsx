...
const CATEGORIES = ["Food", "Tech", "Fashion", "Travel", "Education", "Entertainment"]

export function DiscountsGrid({ discounts: initialDiscounts }: DiscountsGridProps) {
    ...
    <div className="space-y-6">
          ...
          {CATEGORIES.map((category) => (
            <Badge
              ...
              className={`cursor-pointer transition-colors ${
                selectedCategory === category ? "bg-amber-500 hover:bg-amber-600" : "hover:bg-gray-50"
              }`}
      ...
      <div className="border-t pt-4">
        <p className="text-sm text-muted-foreground">
          {initialDiscounts.length} {initialDiscounts.length === 1 ? "discount" : "discounts"} available
        </p>
      </div>
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        {initialDiscounts.length > 0 ? (
            ...
            <Card key={discount.id} className="overflow-hidden hover:shadow-lg transition-shadow">
              ...
              <div className="h-32 bg-gradient-to-br from-gray-50 to-gray-100 flex items-center justify-center p-6">
                {discount.brand_logo ? (
                  ...
                  <div className="text-center">
                    ...
                    <p className="font-bold text-lg text-gray-700">{discount.brand_name}</p>
                  </div>
                )}
              </div>
              ...
              <CardHeader>
                <div className="flex items-start justify-between gap-2 mb-2">
                  <h3 className="font-semibold text-lg">{discount.brand_name}</h3>
                  <Badge className={getCategoryColor(discount.category)}>{discount.category}</Badge>
                </div>
              ...
              <CardContent className="space-y-4">
                ...
                {discount.discount_code && (
                  <div className="space-y-2">
                    <p className="text-xs font-medium text-muted-foreground">Discount Code</p>
                    <div className="flex items-center gap-2">
                      <div className="flex-1 rounded-lg border-2 border-dashed border-amber-300 bg-amber-50 px-4 py-3 font-mono font-bold text-center">
                        {discount.discount_code}
                      </div>
                      <Button
                        size="icon"
                        variant="outline"
                        onClick={() => handleCopyCode(discount.discount_code!, discount.id)}
                ...
                <div className="flex items-center gap-2 text-sm text-muted-foreground">
                  ...
                  <span>Valid until {format(new Date(discount.valid_until), "MMM dd, yyyy")}</span>
                </div>
          ...
          <div className="col-span-full rounded-lg border-2 border-dashed bg-white p-12 text-center">
            ...
            <p className="text-muted-foreground">No discounts found in this category.</p>
            <Button
              variant="outline"
